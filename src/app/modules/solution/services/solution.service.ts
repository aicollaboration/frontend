...
export class SolutionService {
  ...
  public getSolution(solutionId: number): Observable<SolutionModel> {
    const solutionOfUndefined = this.solutions.find(solution => solution.id == solutionId);

    if (solutionOfUndefined) {
      return of(solutionOfUndefined);
    }

    throw new Error(`Solution with id ${solutionId} not available`);
  }
}
